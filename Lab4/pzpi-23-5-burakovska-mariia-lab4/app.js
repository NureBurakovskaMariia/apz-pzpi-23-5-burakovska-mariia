 ...
 *               role:
 *                 type: string
 *                 enum: [SUPERADMIN, ADMIN, VOLUNTEER, USER]
 *                 example: "USER"
 *     responses:
 *       201:
 ...
 *               name: { type: string }
 *               email: { type: string }
 *               role: { type: string, enum: [SUPERADMIN, ADMIN, VOLUNTEER, USER] }
 *     responses:
 *       200:
 ...
 *     responses:
 *       200:
 *         description: Успішний вхід. Повертає дані користувача.
 *       401:
...
router.post('/login', async (req, res) => {
  try {
    ...
    // Якщо користувача немає або паролі не співпадають
    if (!user || user.password_hash !== password_hash) {
      return res.status(401).json({ message: 'Invalid email or password' });
    }

    // Відділяємо пароль, щоб не відправляти його назад на клієнт в цілях безпеки
    const { password_hash: _, ...safeUserData } = user;
 ...
 * /volunteers/{id}/tasks:
 *   get:
 *     summary: Отримати завдання конкретного волонтера
 *     tags: [Tasks]
 *     parameters:
 *       - in: path
 *         name: id
 ...
 *     responses:
 *       200:
 *         description: Список завдань волонтера.
 */
router.get('/volunteers/:id/tasks', async (req, res) => {
  try {
    const tasks = await taskModel.getTasksByVolunteer(req.params.id);
    res.json(tasks);
  } catch (error) {
 ...
 * /users/{id}/tasks:
 *   get:
 *     summary: Отримати завдання користувача (перевіряє, чи є він волонтером)
 *     tags: [Tasks]
 *     parameters:
 *       - in: path
 *         name: id
 ...
 *         schema:
 *           type: integer
 *         description: ID користувача (users.id) з токена/сесії
 *     responses:
 *       200:
 *         description: Список завдань волонтера (або порожній масив).
 *       500:
 *         description: Помилка сервера.
 */
router.get('/users/:id/tasks', async (req, res) => {
  try {
    ...
    const volunteer = await userModel.getVolunteerByUserId(userId);

    if (!volunteer) {
      return res.json([]);
    }

    const tasks = await taskModel.getTasksByVolunteer(volunteer.id);
    res.json(tasks);
 ...
 * /tasks:
 *   get:
 *     summary: Отримати список усіх завдань (Адміністратор)
 *     tags: [Tasks]
 *     responses:
 *       200:
 ...
 * /admin/users:
 *   get:
 *     summary: Отримати список усіх користувачів (Суперадмін)
 *     tags: [Admin]
 *     responses:
 *       200:
 ...
 * /superadmin/export:
 *   get:
 *     summary: Справжній експорт всієї бази даних у JSON
 */
router.get('/superadmin/export', async (req, res) => {
  try {
    console.log("Починаємо експорт бази даних...");
    const dbData = {};
    ...
    for (const table of tables) {
      try {
        dbData[table] = await knex(table).select('*');
        console.log(`Таблиця '${table}' успішно експортована (${dbData[table].length} записів).`);
      } catch (tableError) {
        console.log(`Помилка з таблицею '${table}': ${tableError.message}`);
        dbData[table] = []; 
      }
    }
    ...
    res.setHeader('Content-disposition', 'attachment; filename=petshelter_backup.json');
    res.setHeader('Content-type', 'application/json');
    res.write(JSON.stringify(backupData, null, 2));
    res.end();
  } catch (error) {
    console.error("FATAL Export error:", error);
    res.status(500).json({ message: 'Помилка при експорті бази даних' });
  }
});
 ...
 * /superadmin/import:
 *   post:
 *     summary: Справжнє відновлення бази даних із JSON файлу
 */
router.post('/superadmin/import', upload.single('backup'), async (req, res) => {
  if (!req.file) {
  ...
  try {
    ...
    await knex.transaction(async (trx) => {
      await trx('tasks').del();
      await trx('adoption_requests').del();
      await trx('medical_records').del();
      await trx('sensor_readings').del();
      await trx('donations').del();
      await trx('volunteers').del();
      await trx('animals').del();
      await trx('users').del();

      if (data.users?.length) await trx('users').insert(data.users);
      if (data.animals?.length) await trx('animals').insert(data.animals);
      if (data.volunteers?.length) await trx('volunteers').insert(data.volunteers);
      if (data.donations?.length) await trx('donations').insert(data.donations);
      if (data.medical_records?.length) await trx('medical_records').insert(data.medical_records);
      if (data.sensor_readings?.length) await trx('sensor_readings').insert(data.sensor_readings);
      if (data.adoption_requests?.length) await trx('adoption_requests').insert(data.adoption_requests);
      if (data.tasks?.length) await trx('tasks').insert(data.tasks);

      const tables = [
        'users', 'animals', 'volunteers', 'donations',
        'medical_records', 'sensor_readings', 'adoption_requests', 'tasks'
      ];
    ...
    res.status(500).json({ message: 'Помилка при відновленні даних. Перевірте цілісність файлу.' });
  }
});
 ...
 *   get:
 *     summary: Отримати технічну статистику бази даних
 */
router.get('/superadmin/system-stats', async (req, res) => {
  try {
    const tables = [
      'users',
      'animals',
    ...
    for (const table of tables) {
      ...
      stats.push({
        table_name: table,
        row_count: parseInt(result.count, 10)
      });
    }
    ...
    res.status(500).json({
      message: 'Помилка отримання системних даних'
    });
  }
});
